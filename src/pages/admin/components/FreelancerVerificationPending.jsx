...
const FreelancerVerificationPending = () => {
  ...
  const {
    ...
    queryKey: ["freelancer-carification", currentPage, tableLimit],
    queryFn: fetchData,
...
export default FreelancerVerificationPending;
...
const UserTableCard = ({ data = {} }) => {
  ...
  const verifyFunc = async () => {
    try {
      const response = await axiosInstance.post("/admin/freelancer/verify", {
  ...
  const verifyMutate = useMutation({
    ...
    onSuccess: (resData) => {
      ...
      queryClient.invalidateQueries(['freelancer-carification', 'freelancer-data', 'dashboard-action-data']);
    },
  });
